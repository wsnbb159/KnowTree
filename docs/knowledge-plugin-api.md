...
知树页面顶部的「插件」按钮打开的就是插件广场。它没有后端，架构是两段式的：

| 内容 | 位置 | 版本策略 | 为什么 |
| --- | --- | --- | --- |
| 清单 `registry.json` | 仓库 `plugins/`，经 jsDelivr 读取 | `@main` | 它必须能**更新** —— 加新插件不该需要改代码重新部署 |
| 插件脚本 `xxx.js` | 同上 | **完整 commit sha** | 它必须**不可变** —— 见下方实测教训 |

发布一个新插件只要三步：

1. 在 `plugins/` 下写一个自注册脚本（照抄 `plugins/physics.js` 的结构）；
2. 在 `plugins/registry.json` 的 `plugins` 数组里加一条，`script` 指向该文件；
3. 跑 `node scripts/pin-plugins.mjs`，提交推送。

**为什么脚本必须钉 commit 而不是用 `@main`（这是实测踩出来的）：**

jsDelivr 对**分支名**的缓存可以长达数天。改完 `plugins/physics.js` 后实测：
按 `@main` 取回来的仍是旧版本，按完整 commit sha 取则立即是新版本。
这个坑的恶劣之处在于**不报错** —— 用户点「安装」会装到一个过时的插件，
界面上一切正常，你在本地怎么测都是对的（本地 `cache: no-store` 也绕不过 CDN 的服务端缓存）。

所以 `pin-plugins.mjs` 把「清单里的 script 引用」与「页面内置兜底清单里的同一引用」
一次性钉到当前 commit —— 用脚本而不是手改，是因为这两处一旦不一致，
就会出现「线上清单是新的、离线兜底是旧的」这种极难发现的错位。

**为什么清单本身必须留在 `@main`：** 如果清单也钉 sha，那加一个插件就得改前端代码
重新部署一次 —— 广场就退化成了写死的列表，失去意义。
清单的分支缓存延迟（几分钟）由页面内置的兜底清单兜住，用户不会看到空白。
