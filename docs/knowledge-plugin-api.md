...
完整的可运行样例见仓库里的 [`public/plugins/physics.js`](../public/plugins/physics.js)，
...
**但更值得知道的是：不带画像会削弱归因的表达力。** 这是实测出来的：
物理插件最初没带画像时，那道斜面连接体题得到的是
「问题就在摩擦力本身 · 波及 0 个 · 错因＝读题失误」——
结论没错，但完全没讲出这道题真正的教学意义。
补上画像（把「受力分析」52 分、「摩擦力」58 分）后，
归因沿依赖链回溯到最靠前的失效边界，因果链变成「受力分析 → 摩擦力」，
这才是这道题想说的那句话。

两个注意点：

- **画像必须覆盖所有节点。** 缺失的节点会被当成「没掌握」（这是对的行为：没数据就是没掌握），
  而不是回退到某个默认分。想省事可以像物理插件那样：先按节点批量生成一份基线，再覆盖几个要设低分的。
- **想让归因「回溯到上游」，至少要让上游也低于 60 分。** 只把考点设为低分，
  归因就停在考点自身 —— 算法选的是「通往考点的路径上、最靠近考点的失效边界」。
...
知树页面顶部的「插件」按钮打开的就是插件广场。它没有后端，由两个文件支撑：

| 内容 | 位置 | 说明 |
| --- | --- | --- |
| 清单 `registry.json` | `public/plugins/`，随站点部署 | 页面以**同源相对路径**读取，部署即最新 |
| 插件脚本 `xxx.js` | 站内同目录，或任意外部地址 | `script` 支持相对路径与完整 URL |

发布一个站内插件只要两步：

1. 在 `public/plugins/` 下写一个自注册脚本（照抄 `physics.js` 的结构）；
2. 在 `public/plugins/registry.json` 的 `plugins` 数组里加一条，`script` 填相对路径，重新部署。

外部插件同理，只是 `script` 填完整 URL —— 前提是该地址**可跨域**且内容**不可变**（见下）。

**为什么清单不托管到 CDN（这是实测撞出来的，值得记一笔）：**

我们最初把清单放在 jsDelivr 上，想着「加插件不用重新部署」。实测发现：
jsDelivr 对**分支名**的缓存长达数天，而且**刷不掉** ——
`purge.jsdelivr.net` 接口无效，query string 也不参与缓存键
（无论加多少 `?t=Date.now()` 都拿回同一份旧内容）。
后果是「改了清单，线上还是旧内容」，而界面上一切正常，本地怎么测都对
（`cache: 'no-store'` 也绕不过 CDN 的服务端缓存）—— 最难查的那一类问题。

改用同源相对路径后，部署即最新，跨域问题也一并消失。

**如果确实要用外部 CDN 托管脚本，必须按完整 commit sha 引用，不要用分支名。**
实测：改完脚本后按 `@main` 取回的仍是旧版本，按完整 sha 取则立即是新版本。
仓库里的 `scripts/pin-plugins.mjs` 就是干这件事的 ——
它会把<strong>清单</strong>与<strong>页面内置兜底清单</strong>两处的外部引用一起钉到当前 commit。
用脚本而不是手改，是因为这两处一旦不一致，就会出现「线上清单是新的、离线兜底是旧的」
这种极难发现的错位。
